/* This verstion is copied from 2014/1/5/Acceleration. 
 * This code is used to output motor motion string to processing, to analyse if 
 * motor acceleration algorithmus is possibile
 * if motor1 move one step, Serial print "1", motor2 print "2"
 *
 *
 * data: 2014_1_16
 * written by Gfast
...
static void setPenAngle(int pen_angle) {
  if(posz!=pen_angle) {
    ...
    if(posz == 170){ //pen move up
      s1.write(100); //slowly counterclock rotate
    ...
    else if(posz == 10){ //pen move down
      s1.write(82); //slowly clock rotate
...
void delayMashine2(long ad1, long i){
  ...
  if(i < mid){
    ...
    delayMax = delayMax - mid*15 + (i - mid)*15;
  }
