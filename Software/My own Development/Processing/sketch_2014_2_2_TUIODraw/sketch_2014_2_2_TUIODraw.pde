...
Suline[] Suli = new Suline[10000]; //initial a Suline object array
...
int x = width / 2; //the uptodate x position on this frame
int y = height / 2; //the uptodate y position on this frame
int x_Old = width / 2; //the x position on last frame
int y_Old = height / 2; //the y position on last frame
...
{  
  ...
  for (int i=0;i<message;i++) {
    ...
    x = width - tobj.getScreenX(width); //"width" get the width of the window
    ...
    if (tobj.getSymbolID() == 1) { //if drawing      
      ...
      Suli[linie-1] = new Suline(x,y,x_Old,y_Old); //store the line have been created
    }
      ...
      GoHome();
      //Suli[linie-1] = new Suline(x,y,x_Old,y_Old); //store the line have been created
      Suli[linie-1] = new Suline(0,0,0,0); //store the line have been created
    }
    else {
...
//this class is used to store the line that been / planed to drawn, which is created by 
//reaTIVIsion
class Suline {
  int x;
  int y;
  ...
  Suline(int tempX, int tempY, int tempX_old, int tempY_old) {
    x = tempX;
    y = tempY;
    x_old = tempX_old;
    y_old = tempY_old;
  ...
  //draw the stored line
  void drawLine(int x ,int y, int x_old, int y_old){
